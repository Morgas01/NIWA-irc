(function(µ,SMOD,GMOD,HMOD,SC){
	...
	let xdccManager=new DownloadManager({
		...
		{
			return checkXdccDownload(download);
		},
		autoTrigger:true
	});
	...
	xdccManager.parseMessage=function(message,network,user)
	{
		...
		if(match)
		{
			...
			{
				case "SEND":
					offer.quotedFilename=offer.quotedArgument;
					...
					if(xdccRequest)
					{
						...
						else
						{
							...
							{
								catchOffer=worker.ask(xdccRequest.download.context,"openIrcOffer",{
									offer:{
										network:network,
										user:user,
										filename:offer.quotedFilename,
										ip:offer.ip,
										port:offer.port,
									},
									originalDownloadName:xdccRequest.download.name
								});
							}
							else
							{
	...
	{
		return checkXdccDownload(xdccDownload)
		.then(()=>config.ready)
	...
	xdccManager.downloadDCC=function(signal,download)
	{
		...
		new SC.Promise([
			config.ready,
			SC.FileUtil.enshureDir(download.filepath).then(µ.constantFunctions.scope,function(e)
			{
				return signal.reject(`filepath (${this.getAbsolutePath()}) could not be created: ${SC.errorSerializer(e)}`);
			})
		])
		.then((config,folder)=>
		{
			...
			if(getConfigValue(config,download.dataSource.resume,["DCC","resume"]))
			{
				...
				.then(stats=>new Promise((resolve,reject)=>
				{
					...
					this.updateDownload(download);
					SC.ircManager.message(download.dataSource.network,download.dataSource.user,`DCC RESUME ${offer.quotedFilename} ${offer.port} ${stats.size}`);
				}),
	...
	let checkXdccDownload=function(download)
	{
		let errors=[];

		if(download.dataSource.ip)
		{
			errors=checkDccDownload(download);
		}
		else
		{
			if(!download.dataSource.network) errors.push("no network");
			if(!download.dataSource.user) errors.push("no user");
			if(!download.dataSource.packnumber) errors.push("no packnumber");
			if(!download.filepath) errors.push("no filepath");
		}
