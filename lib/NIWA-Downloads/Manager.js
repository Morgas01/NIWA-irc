(function(µ,SMOD,GMOD,HMOD,SC){
	...
	var MANAGER=module.exports=µ.Class({
		...
		{
			SC.rescope.all(this,["isDownloadNotRunning","receiveDownload","updateDelegatedDownload"]);
			options=SC.adopt({
				eventName:"downloads",
    	...
    	isDownloadNotRunning:function(download)
    	{
    		...
    		{
    			if(running.ID===download.ID&&running.remoteID===download.remoteID) return false;
    		}
    		return true;
    	},
    	startDownload:function(download)
    	{
    		...
    		.then(()=>
    		{
				...
				var runningInfo={promise:null,delegate:false};
				this.runningDownloadMap.set(download,runningInfo);
				runningInfo.promise=new SC.Promise(this.download,{args:[download]});
				runningInfo.promise.always(()=>
				{
					this.runningDownloadMap.delete(download);
					this.dbConnector.then(dbc=>
					{
						if(download.appName)dbc.delete(this.DBClassDictionary[download.objectType],[download]).catch(e=>µ.logger.error({error:e},"failed to delete completed download"));
						else dbc.save(download).catch(e=>µ.logger.error({error:e},"failed to save completed download"));
						this.updateDownload(download);
					});
				});
				runningInfo.promise.catch(function(error)
				{
    	...
    	updateDelegatedDownload:function(update)//,appName) TODO check appName?
    	{
    		...
    		if(runningDownload)
    		{
    			var runningInfo=this.runningDownloadMap.get(runningInfo);
    			if(runningInfo.onUpdate) runningInfo.onUpdate(update,runningDownload)
    			else runningDownload.fromJSON(update);
    			this.updateDownload(runningDownload);
    		}
    		else
    		{
