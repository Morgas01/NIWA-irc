(function(µ,SMOD,GMOD,HMOD,SC){
	...
	let configApi=require("../rest/config");
	...
	let manager=module.exports={
		...
		join:function(network,channel)
		{
			return getClient(network).then(function(client)
			{
				...
				if (index==-1)
				{
					return new Promise(rs=>client.join(lowerChannel,rs));
				}
			});
		},
	...
	let createClient=function(network,user,password)
	{
		if(!clients.has(network))
		{
			let clienPromise=new SC.Promise([getCredentials(network,user,password),configApi.ready])
