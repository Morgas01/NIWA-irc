(function(µ,SMOD,GMOD,HMOD,SC){
	...
	var xdccManager=require("./xdccManager");
	...
	var checkConnection=client=>client.conn.readystate=="closed";
	...
	var manager=module.exports={
		...
		connect:function(network,user,password)
		{
			if(clients.has(network))
			{
				...
				{
					if(checkConnection(client)) return Promise.reject("client already connected: "+network);
					client.end();
	...
	var createClient=function(network,user,password)
	{
		if(!clients.has(network))
		{
			return clients.set(network,config.ready
	...
	var getClient=function(network)
	{
		...
		{
			if(!checkConnection(client)) return Promise.reject("client disconnected: "+network);
			return client;
		});
	}
	...
	var addMessage=function(client,sender,receiver,text,type)
    {
    	...
    	if(type==TYPES.CTCP+" privmsg") xdccManager.parseMessage(message,client.opt.server,sender);
